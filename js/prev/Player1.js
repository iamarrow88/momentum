...
const Player1 = {
  ...
  createPlayer() {
    ...
    playerWrapper.addEventListener("click", (e) => {
      let elementClass;
      const elementDataset = e.target.dataset; // содержит любой датасет, нужно выбирать необходимый
      ...
      console.log(`объект в кнопочками для смены содержит: ${Object.keys(Player1.fields.toggleButtons)}`);
      console.log(`elementClass - ${elementClass}`);
      console.log(`Его противоположность - ${Player1.fields.toggleButtons[elementClass]}`);
      console.log("кнопочкi - ");
      console.log(document.querySelectorAll(`[data-button-name=${elementClass}]`));
      if (Object.keys(Player1.fields.toggleButtons).includes(elementDataset.buttonName)) {
        basic.changeElementSvg(document.querySelector(`use[data-button-name=${elementDataset.buttonName}]`), Player1.fields.pathToSVG + Player1.fields.toggleButtons[elementDataset.buttonName], `use-${elementDataset.buttonName}`, `use-${Player1.fields.toggleButtons[elementClass]}`);
        ...
        allElementsWithThisData.forEach((element) => {
          element.dataset.buttonName = Player1.fields.toggleButtons[elementDataset.buttonName];
        });
      }
