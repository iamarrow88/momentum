...
function greet(time) {
  ...
  } else if (allAboutThisPage.timeOfADay !== "night") {
...
function getQuote() {
  const lang = allAboutThisPage.selectedLang;
  if (lang === "en") {
    getValue("https://type.fit/api/quotes").then((value) => {
      const randomNum = getRandomNUm(value.length);
      rewriteTextBlock(value[randomNum]);
    });
  } else {
    getValue("./assets/quotes-ru.json").then((value) => {
      const randomNum = getRandomNUm(value.length);
      rewriteTextBlock(value[randomNum]);
    });
  }
...
function getWeather(lang) {
  ...
  getValue(url).then((weatherObj) => {
    if (weatherObj.message) {
      ...
      const windBlock = document.querySelector(".wind");
...
CITY_NAME.addEventListener("click", () => {
  CITY_NAME.value = "";
...
CITY_NAME.addEventListener("focusout", (event) => {
  CITY_NAME.setAttribute("value", allAboutThisPage.cityName);
});
...
document.querySelector(".hide-block").addEventListener("click", (event) => {
...
document.querySelector(".item__lang").addEventListener("click", (event) => {
...
TO_DO_INPUT.addEventListener("change", () => {
